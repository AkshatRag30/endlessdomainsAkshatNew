...
import styles from './BNBDomainDetails.module.scss'
...
export function BNBDomainDetails({ domain, owner, expiry }: BNBDomainDetailsProps) {
  const [copied, setCopied] = useState(false)
  const { name, tld } = parseDomain(domain)
  const bnbProvider = getDomainProvider('BinanceSmartChain')!
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.banner}>
        <div className={styles.avatarWrap}>
          <Image
            src={bnbProvider.image}
            alt={bnbProvider.label}
            width={41}
            height={41}
        ...
        <div className={styles.nameCol}>
          ...
          <p className={styles.providerLabel}>binance smart chain</p>
        </div>
      </div>
      ...
      <dl className={styles.infoRow}>
        ...
        <div className={styles.field}>
          ...
          <dd className={styles.fieldBox}>
            <Image src={bnbProvider.image} alt="" width={22} height={22} aria-hidden="true" />
            <span className={styles.fieldText}>{bnbProvider.label}</span>
          </dd>
        </div>
